...
class BlackjackEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 1:  # hit: add a card to players hand and return
            ...
            self.player.append(draw_card(self.np_random))
            if len(self.player2) == 0 | self.terminated2:
                terminated = True
                if is_bust(self.player):
                    reward = -2.0
                else:
        ...
        elif action == 3: # split: only possible with two cards of same rank, split hand in two, place second bet for second hand, play hands after another
            if self.player[0] == self.player[1] & len(self.player) == 2:
